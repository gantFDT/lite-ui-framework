...
import { EditorPrivateContextProps, CustomModalProps } from './context/EditorPrivateContext';
...
export interface GraphConfig {
  ...
  edgeType: 'reject' | 'approve' | 'cancel';
  defaultNode: object;
  defaultEdge: object;
  plugins: any[];
  animate: boolean;
...
export interface Graph extends EventEmitter {
  ...
  // 更新
  add<T = Node>(type: ItemType, model: NodeModel | EdgeModel | ItemModel): T;
  ...
  find<T = Item>(type: 'node' | 'edge', func: (node: Item) => boolean): T;
  findAll<T = Item>(type: 'node' | 'edge', func: (node: Item) => boolean): T[];
...
export interface Behavior {
  ...
  shouldBegin?(ev?: GraphEvent): boolean;
...
export interface CustomUserData {
  ...
  customModalProps: { [modalType: string]: CustomModalProps };

  setBrushState: (brushState: string) => void;
  setDesignId: (designId: string | null) => void;
  setDesignName: (designName: string | null) => void;
  setWorkflowTemplateData: (workflowTemplateData: WorkflowTemplateData) => void;
  setCustomModalProps: (modalType: string, modalProps: CustomModalProps) => void;
}

export interface CommandDataSource extends CustomUserData {
  graph: Graph | null;
...
export interface Command<P = object, D = CommandDataSource> {
  /** 命令名称 */
  name: string;
  /** 命令参数 */
  params: P;
  /** 是否可以执行 */
  canExecute(dataSource: D): boolean;
  /** 是否应该执行 */
  shouldExecute(dataSource: D): boolean;
  /** 是否可以撤销 */
  canUndo(dataSource: D): boolean;
  /** 初始命令 */
  init(dataSource: D): void;
  /** 执行命令 */
  execute(dataSource: D): void;
  /** 撤销命令 */
  undo(dataSource: D): void;
  /** 命令快捷键 */

import cloneDeep from 'lodash/cloneDeep';
import { EditorEvent } from '../constants';
import { Command, CommandDataSource } from '../interface';

class CommandManager {
  command: {
    [propName: string]: Command;
  };
  commandQueue: Command[];
  commandIndex: number;

  constructor() {
    this.command = {};
    this.commandQueue = [];
    this.commandIndex = 0;
  }

  /** 注册命令 */
  register(name: string, command: Command) {
    this.command[name] = {
      ...command,
      name,
    };
  }

  /** 执行命令 */
  execute(dataSource: CommandDataSource, name: string, params?: object) {
    const Command = this.command[name];

    if (!Command) {
      return;
    }

    const { graph } = dataSource;

    if (!graph) {
      return;
    }
    
    const command = Object.create(Command);

    command.params = cloneDeep(Command.params);

    if (params) {
      command.params = {
        ...command.params,
        ...params,
      };
    }

    if (!command.canExecute(dataSource)) {
      return;
    }

    if (!command.shouldExecute(dataSource)) {
      return;
    }

    command.init(dataSource);

    graph.emit(EditorEvent.onBeforeExecuteCommand, {
      name: command.name,
      params: command.params,
    });

    command.execute(dataSource);

    graph.emit(EditorEvent.onAfterExecuteCommand, {
      name: command.name,
      params: command.params,
    });

    if (!command.canUndo(dataSource)) {
      return;
    }

    const { commandQueue, commandIndex } = this;

    commandQueue.splice(commandIndex, commandQueue.length - commandIndex, command);

    this.commandIndex += 1;
  }

  /** 判断是否可以执行 */
  canExecute(dataSource: CommandDataSource, name: string) {
    return this.command[name].canExecute(dataSource);
  }

  /** 注入是否应该执行 */
  injectShouldExecute(name: string, shouldExecute: (dataSource: CommandDataSource) => boolean) {
    this.command[name].shouldExecute = shouldExecute;
  }
}

export default new CommandManager();
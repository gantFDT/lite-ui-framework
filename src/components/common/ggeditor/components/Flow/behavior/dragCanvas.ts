...
behaviorManager.registerFlowBehavior('flow-drag-canvas', {
  ...
  isCanvas(ev: GraphEvent) {
    const { target } = ev;
    const targetName = target.get('renderType');
    if (targetName == 'canvas') return true;
    else return false;
  },
  isLeftButton(ev: GraphEvent){
    const { event } = ev;
    if(event.button === 0) return true;
    return false;
  },
  onKeyUp(e: GraphEvent) {
    const SPACE = 32;
  ...
  onMouseDown(e: GraphEvent) {
    ...
    if (this.needSpaceDown && !this.keyFlag) return;
    if (this.onlyDragCanvas && !this.isCanvas(e)) return;

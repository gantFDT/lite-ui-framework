...
G6.registerBehavior('align', {
  ...
  onMouseDown(e: GraphEvent){
    const targetName = e.target.get('className');
    if (targetName === 'edge-center-anchor' || targetName === 'edge-control-anchor') {
      this.flag = true;
    }
  },
  onMouseMove(e: GraphEvent){
    const { x: canvasX, y: canvasY, item } = e;
    if(!this.flag) return;
    if(!item) return;
    const auxiliaryLine = item.get('auxiliaryLine') || {};
    
    const { graph } = this;
    const originPoint = { x: canvasX, y: canvasY };
    ...
    nodes.forEach(node => {
      ...
      const calc = <T>(point: Point, lines: Lines, arr: HVLine, axis: string) => {
        for (let name of Object.keys(lines)) {
          const line = lines[name];
          const dis = pointLineDistance(line, [point.x, point.y]);
          if (Math.abs(dis) < tolerance) {
            auxiliaryLine.attachFlag = axis;
            auxiliaryLine.attachTolerance = tolerance;
            if (axis === 'Y') auxiliaryLine.y = originPoint.y - dis;
            if (axis === 'X') auxiliaryLine.x = originPoint.x + dis;
            arr[name].push({ line, dis });
          }
        }
      };
      calc(originPoint, horizontalLines, hLines, 'Y');
      calc(originPoint, verticalLines, vLines, 'X');
    });
    // 排序
    ...
    const drawLine = (lines: any, d: any) => {
      ...
      names.forEach(name => {
        if (lines[name].length > 0) {
          ...
          const xs: number[] = [canvasX, line[0], line[2]];
          const ys: number[] = [canvasY, line[1], line[3]];
          const y = line[1];
          const x = line[0];
          // 将画布转标转换为视口坐标并乘缩放比例
          ...
          auxiliaryLine[`_${name}`] = graph.get('canvas').addShape('path', {
    ...
    item.set('auxiliaryLine', auxiliaryLine);
  },
  onMouseUp(e: GraphEvent){
    ...
    ['HTL', 'HCL', 'HBL', 'VLL', 'VCL', 'VRL'].forEach(lname => {
      if (this.auxiliaryLine['_' + lname]) {
        this.auxiliaryLine['_' + lname] && this.auxiliaryLine['_' + lname].remove();
        this.auxiliaryLine['_' + lname] = null;
      }
    });

    e.item && e.item.set('auxiliaryLine', this.auxiliaryLine);
  },
  onDrag(e: GraphEvent) {

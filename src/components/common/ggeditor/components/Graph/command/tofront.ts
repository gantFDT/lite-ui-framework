...
interface CommandProps extends BaseCommand<CommandParams> {
  toFront(graph: Graph, selectedNodes: Node[]): void;
}
...
const Command: CommandProps = {
  ...
  execute(dataSource) {
    ...
    if (graph) {
      const selectedNodes = this.getSelectedNodes(graph);
      this.toFront(graph, selectedNodes);
    }
  },
};

commandManager.register('toFront', Command);

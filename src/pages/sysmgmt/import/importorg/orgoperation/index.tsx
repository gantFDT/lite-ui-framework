...
function OrgOperstion(props: any) {
  const {
    submitUploadFile,
  } = props;
  ...
  const downloadfile = useCallback((event) => {
    const url = '/api/importSecurityData/getOrgImportTemplate'
  ...
  return (
    <div className={styles.orgoperstion}>
      <div>
        <Button size="small"
          type="primary"
        ...
        <Button size="small"
          ...
          onClick={onSubmit}
        >
          <Icon type='import' />{tr('导入组织数据')}
        </Button>
      </div>
      <div className={styles['orgoperstion-input']}>
        <label>{tr('文件名称')}</label>
...
export default connect(({ settings }: any) => ({
  ...
  submitUploadFile: (payload: any) => dispatch({ type: 'importorg/submitFile', payload }),
})
)(OrgOperstion)
